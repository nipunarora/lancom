 ...
 config_statement 
 	: 
 	 ...
 	 catch (DataFormatException dfe)
 	 {
 	   ...
 	   if (route != null)
 	   {
 	   route.configure("route.xml",$route_cmd.text,"route");
 	   }
 	...
 	if(r!=null)
 	{
 	r.configure("route.xml",$route_cmd2.text,"route");
 	}
 	}
 	|
 	{Interface ifc=null;}
 ...
 */
 ...
 @final
 {
 	...
 	if_statement_list)? 'endif'
 	{
		...
		if(input.LA(1) == ELSE)
		{
			match(input,ELSE,FOLLOW_ELSE_in_condition_statement862);  
 	...
 	if_statement_list 
	{
 	...
 	'endwhile'
 	{
 		if(!g_ignore_stmt)
 		{
 			...
 			match(input,OPAREN,FOLLOW_OPAREN_in_iteration_statement923);  
 			val = equality_expr();
 			if (((IntType)val.sym.lookupValue()).getValue() == 1)
   			{
   				...
   				{
   					match(input,CPAREN,FOLLOW_CPAREN_in_iteration_statement937);  
   					if_statement_list();
   				}
   			}
   			else
   			...
   			{
   				match(input,ENDWHILE,FOLLOW_ENDWHILE_in_iteration_statement957);  
   			}
 		}
  		g_ifStack.pop();
  		g_ignore_stmt = false;
 			...
 			if(!g_ignore_stmt)
 			{
				...
				while(iter.hasNext() && $e.sym != null)
				{
					...
					if($e.sym.getType().equals("int_type_t") || $e.sym.getType().equals("char_type_t"))
					{
						...
						currentScope.setSymbolValue(key, asn.getSymbol().lookupValue());
					}
					else
					{
