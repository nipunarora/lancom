 ...
 */
 ...
 @final
 {
 	...
 	if_statement_list)? 'endif'
 	{
		...
		if(input.LA(1) == ELSE)
		{
			match(input,ELSE,FOLLOW_ELSE_in_condition_statement296);  
 	...
 	if_statement_list 
	{
 	...
 	'endwhile'
 	{
 		if(!g_ignore_stmt)
 		{
 			...
 			match(input,OPAREN,FOLLOW_OPAREN_in_iteration_statement363);  
 			val = equality_expr();
 			if (((IntType)val.sym.lookupValue()).getValue() == 1)
   			{
   				...
   				{
   					match(input,CPAREN,FOLLOW_CPAREN_in_iteration_statement376);  
   					if_statement_list();
   				}
   			}
   			else
   			...
   			{
   				match(input,ENDWHILE,FOLLOW_ENDWHILE_in_iteration_statement394);  
   			}
 		}
  		g_ifStack.pop();
  		g_ignore_stmt = false;
		...
		{	
			...
			else if (ip_object_used == false  && gw_object_used ==true)
			{
				...
				/* Check for type error in netmask  */
			      	try
			      	{
					f(s.getType().equals ("ipaddr_t") != true) 
				  	{ 
				  		throw (new DataFormatException("rule : route:host"));
				  	}
				}
