...
class Scope {
   ...
   /* symbolMap in scope */
    ...
    public int setSymbolValue(String name, Object val) 
    {
        Symbol s = symbolMap.get(name);
	  ...
	  /* First look in the current scope */
        ...
        {
		s.setValue(val);
		System.out.println("setting value to: " + s.getType());
		System.out.println("setting value to: " + s.getString());
        }
	  
        /* if not, then check parent's scope */
        if ( parent != null ) 
        {
            parent.setSymbolValue(name, val);
        }
